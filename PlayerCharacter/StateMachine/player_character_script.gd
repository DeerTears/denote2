...
@export var use_action: StringName = "play_use_action"
...
func _unhandled_key_input(event: InputEvent) -> void:
	if event.is_action_pressed("use"):
		call_use_on_overlapping_bodies()
...
@onready var picker: RayCast3D = %Picker

func call_use_on_overlapping_bodies() -> void:
	if picker.is_colliding():
		var body: Node = picker.get_collider()
